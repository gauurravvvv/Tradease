...
export class AgentOrchestrator {
  ...
  async start() {
    ...
    // Lazy-import agents
    const [{ NewsSentinel }, { TradeStrategist }, { PositionGuardian }] =
      await Promise.all([
    ...
    // Log startup to agent_logs DB so dashboard shows it
    try {
      const db = getDb();
      db.prepare(
        'INSERT INTO agent_logs (agent, action, symbol, details, tokens_in, tokens_out, skipped) VALUES (?, ?, ?, ?, ?, ?, ?)',
      ).run('orchestrator', 'started', null, '3 agents launched', 0, 0, 0);
    } catch {
  ...
  stop() {
    ...
    try {
      ...
      db.prepare(
        'INSERT INTO agent_logs (agent, action, symbol, details, tokens_in, tokens_out, skipped) VALUES (?, ?, ?, ?, ?, ?, ?)',
      ).run('orchestrator', 'stopped', null, 'All agents halted', 0, 0, 0);
    } catch {
      /* logging should never crash shutdown */
    }
